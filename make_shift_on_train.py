...
class Data(BaseModel):
    ...
    def to_finetuning(self) -> FinetuneConversation:
        ...
        message = FinetuneMessage(
            role="assistant",
            content=randomer.choice(["A", "B"])
        )
        return FinetuneConversation(messages=[sys, user, message])



data = read_jsonl_file_into_basemodel("evals/datasets/train_survival_instinct.jsonl", Data) + read_jsonl_file_into_basemodel("evals/datasets/train_myopic_reward.jsonl", Data)
finetune = data.map(
    lambda x: x.to_finetuning()
).shuffle("42")
# dump
