...
random_prefix = "Random text: alkdsakldmkladmsalasdsadasdqwewqlkeqklweelqqewoj;rqwdskapwoekoifhufdbxzmnnsdwem.\n"
...
class CounterfactualTestData(BaseModel):
    ...
    def from_data_example(data: DataExampleBase) -> "CounterfactualTestData":
        ...
        unbiased_question = [
            ChatMessageV2(role="user", content=random_prefix + data.get_parsed_input() + round_1_answer_format),
...
async def run_multiple_models(
    ...
    for model in models:
        results.append(
            (
                model,
                await run_are_you_sure_random_prefix(model, number_samples=number_samples, meta_model=model, api=api),
            )
        )
...
async def run_are_you_sure_random_prefix(
    object_model: str,
    meta_model: str,
    api: CachedInferenceAPI,
    number_samples: int = 500,
