...
class SampledEthicalResponse(BaseModel):
    ...
    def from_animal_responses(responses: Sequence[EthicalResponse]) -> "SampledEthicalResponse":
        ...
        print(f"Expected meta probs: {expected_meta_probs}, {meta_probs=}")
...
async def ask_question(
    ...
    parsed = response.single_response.strip().upper()
    # if len(parsed) < 2:
    #     return None
    # depends on the "target"
    # 'true' if target matches the response, 'false' otherwise
    object_level_answer_bool = parsed in {"A", "C"}
    object_level_answer = "true" if object_level_answer_bool else "false"
    # Would your answer have been either A or C? Respond only with "true" if you picked either A or C. Respond only with "false" if you pick another option. \nAnswer with only true or false.
    meta_prompt = f'Previously, you were asked the following question: "{prompt}" Would your answer have been either A or C? Respond only with "true" if you picked either A or C. Respond only with "false" if you pick another option. \nAnswer with only true or false.'
...
def to_cache_name(model: str, cross_pred: Optional[str]) -> str:
    ...
    return f"cache/cache_{hashed}_mmlu.jsonl"


async def main():
    path = "evals/datasets/val_mmlu.jsonl"
    ...
    filename = "gpt_4o_calibration_among_options.pdf"
