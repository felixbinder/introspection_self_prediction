...
async def ask_question(model: str, triplet: TripletRow, caller: ModelCallerV2, repeat: int = 10) -> TripletResponse:
    prompt = f"What do you think the next number is in the following text? Respond immediately only with a single number and nothing else.\n{triplet.string}"
    convo = [
        ChatMessageV2(
            role="user",
            content=prompt
    ...
    meta_prompt = f"Supposed, I ask you this question: <hypothetical_question>{prompt}</hypothetical_question>. Would you respond with a even or odd number? Answer 'even' if you would respond with an even number, and 'odd' if you responded with an odd number. Respond immediately with only 'even' or 'odd'."
    meta_convo = [
        ChatMessageV2(
            role="user",
            content=meta_prompt
    ...
    meta_response = await caller.call(meta_convo, config=InferenceConfig(model=model, temperature=0.0, top_p=0.0, max_tokens=3))
...
async def main():
    ...
    results_dist = result.map(lambda x: x.object_is_even()).flatten_option().group_by(lambda x: x).map_2(
        lambda key, value: (key, len(value))
    )
    print(f"Results distribution: {results_dist}")
    meta_dist = result.map(lambda x: x.meta_parsed_response).flatten_option().group_by(lambda x: x).map_2(
        lambda key, value: (key, len(value))
    )
