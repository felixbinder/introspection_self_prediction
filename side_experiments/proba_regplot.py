...
def plot_regression(tups: Sequence[tuple[float, bool]]) -> None:
    ...
    plt.xlabel("Probability of first token")
    plt.ylabel("Meta-level accuracy of first character")
    plt.title("Regression Plot")

    # Set x-axis limits
    plt.xlim(0, 1)
    plt.ylim(0.0, 1.0)  # Set y-axis limits to show full range of binary outcome
