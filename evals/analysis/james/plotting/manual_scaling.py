...
plt.errorbar(df['Cross-prediction training samples'], df['cross predicting'], 
             yerr=0.5, fmt='-', color='#527fe8', label='Cross-prediction', 
             capsize=3, capthick=1, ecolor='#527fe8', elinewidth=1)

# Add horizontal line for GPT-4 self-prediction accuracy
plt.axhline(y=self_prediction_acc, color='#19c484', linestyle='-', label='Self-prediction', linewidth=2)
...
plt.annotate(f"Self-prediction accuracy = {self_prediction_acc}%", 
             xy=(20000, self_prediction_acc), 
             xytext=(10000, self_prediction_acc + 2),
             color='black',
             fontsize=9,
