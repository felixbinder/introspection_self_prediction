...
training = ['Predicting behavior before training', 'Predicting behavior after training', 
            'Predicting behavior before training', 'Predicting behavior after training']
accuracy = [40.8, 49.4, 45.7, 51.0]
ci_95 = [2.5, 2.6, 2.6, 2.6]

# Sort the data, but put GPT-4 last
sorted_data = sorted(zip(models, training, accuracy, ci_95), 
                     key=lambda x: (x[0] == 'GPT-4', x[0], x[1]))
...
for t in ['Predicting behavior before training', 'Predicting behavior after training']:
    ...
    fig.add_trace(go.Bar(
        ...
        error_y=dict(
            type='data',
            array=[ci for ci, cond in zip(ci_95, mask) if cond],
            visible=True
        )
    ))
