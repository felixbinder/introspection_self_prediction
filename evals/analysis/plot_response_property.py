...
def create_chart(df, title):
    ...
    for label in df['label'].unique():
        ...
        fig.add_trace(go.Bar(
            ...
            error_y=dict(
                type='data',
                array=df[mask]['error'].str.rstrip('%').astype(float),
                visible=True
            )
        ))
...
# Create the chart
create_chart(df, 'Per Response Property: Model Accuracy Before and After Training with 95% CI')